...
function stripReasoning(text) {
  return (text || '')
    ...
    // An answer cut off by max_tokens can open <think> and never close it, so the
    // paired pattern above matches nothing and the entire reasoning blob survives
    // as if it were the reply. Drop an unterminated block too: better to return
    // nothing (the caller's fallback handles it) than to publish the thinking.
    .replace(/<think>[\s\S]*$/i, '')
    .trim();
}
