...
**Two features:**
1. **Weekly cron** — posts a *Четверговая разминка для мозга* every Thursday at 09:00 UTC (7 PM Brisbane / AEST UTC+10). The puzzle is generated by the LLM on each run (recent puzzles are tracked in Redis to avoid repeats); `content/posts.json` is kept only as a fallback if generation fails.
2. **Conversational AI** — Пульсик responds when @mentioned in the group chat, powered by Groq (GPT-OSS 120B, falling back to Qwen3 32B) with per-user memory and rolling channel context stored in Redis.
...
| `GROQ_FALLBACK_MODEL` | No | Model used when the primary errors. Default: `qwen/qwen3-32b` |
...
The weekly puzzle is normally generated by the LLM. `content/posts.json` is only used when generation fails (API outage, malformed output). To add fallback entries:
